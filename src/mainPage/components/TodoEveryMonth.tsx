...
import { todoEMDataState } from "../../recoil/userData";
...
export const TodoEveryMonth: React.FC = () => {
  const [todoEMDataG, setTodoEMDataG] = useRecoilState(todoEMDataState);
  ...
  const onClickSend = () => {
    if (contentValue !== "") {
      setTodoEMDataG([...todoEMDataG, contentValue]);
  ...
  const onClickCancel = (idx: number) => {
    setTodoEMDataG(todoEMDataG.filter((i, id) => idx !== id));
  ...
  const onPressEnterEdit = (
    ...
    if (e.key === "Enter") {
      editValue !== "" &&
        setTodoEMDataG(
          todoEMDataG.map((i, id) => (id === idx ? editValue : i))
        );
      setEditValue("");
      document.getElementById(`editTodoEM${idx}`)?.blur();
    }
  };
  const onToggleEdit = (idx: number) => {
    setToggleEdit(toggleEdit.map((i, id) => (id === idx ? !i : false)));
    document.getElementById(`editTodoEM${idx}`)?.focus();
  };
  const onBlurEdit = (idx: number) => {
    editValue !== "" &&
      setTodoEMDataG(todoEMDataG.map((i, id) => (id === idx ? editValue : i)));
    setEditValue("");
