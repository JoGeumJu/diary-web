...
import { todoEDDataState } from "../../recoil/userData";
...
export const TodoEveryday: React.FC = () => {
  const [todoEDDataG, setTodoEDDataG] = useRecoilState(todoEDDataState);
  ...
  const onClickSend = () => {
    if (contentValue !== "") {
      setTodoEDDataG([...todoEDDataG, contentValue]);
  ...
  const onClickCancel = (idx: number) => {
    setTodoEDDataG(todoEDDataG.filter((i, id) => idx !== id));
  ...
  const onPressEnterEdit = (
    ...
    if (e.key === "Enter") {
      editValue !== "" &&
        setTodoEDDataG(
          todoEDDataG.map((i, id) => (id === idx ? editValue : i))
        );
      setEditValue("");
      document.getElementById(`editTodoED${idx}`)?.blur();
    }
  };
  const onToggleEdit = (idx: number) => {
    setToggleEdit(toggleEdit.map((i, id) => (id === idx ? !i : false)));
    document.getElementById(`editTodoED${idx}`)?.focus();
  };
  const onBlurEdit = (idx: number) => {
    editValue !== "" &&
      setTodoEDDataG(todoEDDataG.map((i, id) => (id === idx ? editValue : i)));
    setEditValue("");
