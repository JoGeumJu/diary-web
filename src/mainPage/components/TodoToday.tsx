...
import { todoTDataState } from "../../recoil/userData";
...
export const TodoToday: React.FC = () => {
  const [todoTDataG, setTodoTDataG] = useRecoilState(todoTDataState);
  ...
  const onClickSend = () => {
    if (contentValue !== "") {
      setTodoTDataG([...todoTDataG, contentValue]);
  ...
  const onClickCancel = (idx: number) => {
    setTodoTDataG(todoTDataG.filter((i, id) => idx !== id));
  ...
  const onPressEnterEdit = (
    ...
    if (e.key === "Enter") {
      editValue !== "" &&
        setTodoTDataG(todoTDataG.map((i, id) => (id === idx ? editValue : i)));
      setEditValue("");
      document.getElementById(`editTodoT${idx}`)?.blur();
    }
  };
  const onToggleEdit = (idx: number) => {
    setToggleEdit(toggleEdit.map((i, id) => (id === idx ? !i : false)));
    document.getElementById(`editTodoT${idx}`)?.focus();
  };
